...
const AgradecimentoForm = ({ onBack }) => {
    ...
    const handleGeneratePDF = () => {
        ...
        // --- CABEÇALHO OFICIAL ---
        // Logo on the left (X=8, Y=10, Width=30, Height=30)
        doc.addImage('/logo-instituicao.png', 'PNG', 8, 10, 22, 22);

        // Text block to the right: starts at X=50, total width=145, center is 122.5
        ...
        // Ensure currentY is below the logo bottom (Y=40)
        currentY = Math.max(currentY + (splitVolunteer.length * 3) + 3, 39);

        // --- BLOCO DE ENDEREÇOS — JUSTIFICADO, bloco homogêneo (igual à imagem) ---
        const addrWidth = 180; // largura do bloco (de X=15 até X=195)
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7.5);
        ...
        currentY += 10;
        doc.setFont('helvetica', 'normal');
        const para1 = `Com nossos sinceros cumprimentos, agradecemos, em nome dos idosos residentes no Abrigo Bezerra de Menezes, a sua colaboração realizada em favor das nossas causas sociais.`;
        ...
        const descClean = description ? description.toLowerCase().replace(/\s+/g, ' ').trim() : '________________________________________________';
        const para2 = `Doando-nos ${descClean} como gesto de amor ao próximo.`;
        const splitPara2 = doc.splitTextToSize('     ' + para2, 165);
        doc.text(splitPara2, 20, currentY, { maxWidth: 165, align: 'justify' });

        currentY += (splitPara2.length * 5.5) + 4;
        ...
        doc.save(`Carta_Agradecimento_${donorData.codigo}.pdf`);
    };
    ...
    return (
        <div className="main-content-layout carta-agrad-premium" style={{ flexDirection: 'column' }}>
            <style dangerouslySetInnerHTML={{
                ...
                /* Preview sheet resembling A4 paper */
                ...
                /* Linha superior: logo + bloco de textos */
                .ch-top-row {
                    display: flex;
                ...
                /* Bloco institucional (nome, CNPJ, textos legais) — centralizado */
                .ch-inst-block {
                    flex: 1;
                    text-align: center;
                }
                ...
                {donorData.nome ? (
                    <>
                        <div className="carta-preview-container">
                            <div className="carta-sheet">
                                ...
                                {/* ============================================================
                                    ...
                                    Linha 2: bloco de endereços alinhado à esquerda
                                ...
                                <div className="carta-content" style={{ marginTop: '8px' }}>
                                    ...
                                    <p style={{ textAlign: 'justify', margin: '0 0 10px 0', textIndent: '30px' }}>
                                        Com nossos sinceros cumprimentos, agradecemos, em nome dos idosos residentes no Abrigo Bezerra de
                                        Menezes, a sua colaboração realizada em favor das nossas causas sociais.
                                    </p>
                                    <p style={{ textAlign: 'justify', margin: '0 0 10px 0', textIndent: '30px' }}>
                                        Doando-nos <strong>{description ? description.toLowerCase() : '________________________________________________'}</strong> como gesto de amor ao próximo.
                                    </p>
                        ...
                        <div style={{ textAlign: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '24px' }}>
                            <button className="carta-btn carta-btn-primary" onClick={handleGeneratePDF} style={{ padding: '12px 30px', margin: '0 auto', fontSize: '1rem', background: '#e11d48' }}>
                                Imprimir
                            </button>
                        </div>
                    </>
