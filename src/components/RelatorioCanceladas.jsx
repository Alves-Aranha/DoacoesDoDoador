...
const RelatorioCanceladas = () => {
    ...
    const handlePrint = async (doacao, type) => {
        let itens = doacao.itens_doacao || [];
        if (!itens.length && doacao.codigo_doacao) {
            const { data: itemData } = await supabase.from('itens_doacao').select('*').eq('id_doacao', doacao.codigo_doacao);
            itens = itemData || [];
        }
